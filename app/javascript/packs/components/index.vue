<template>
  <div class="inner">
    ...
    <div>
      <ul class="collection">
        <li v-for="task in tasks" v-if="!task.is_done" v-bind:id="'row_task_' + task.id" v-bind:key="task.id" class="collection-item">
  ...
  export default {
    ...
    methods: {
      ...
      moveFinishedTask: function(task_id) {
        var el = document.querySelector('#row_task_' + task.id);
        //DOMをクローンしておく
        var el_clone = el.cloneNode(true);
        // 未完了の方を先に非表示にする
        el.classList.add('display_none');
        // 諸々のスタイルなどを足して完了済みに追加
        el_clone.getElementByTagName('input')[0].checked = 'checked';
        el_clone.getElementByTagName('span')[0].classList.add = 'line-through';
